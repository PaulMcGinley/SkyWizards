...
AngryMushroom::AngryMushroom(Player *player, sf::Vector2f spawnPosition, const float viewRange, const float moveSpeed, const int health)
        ...
        // Define the animation sequences for the AngryMushroom
        SetAnimationSequences( {
                {AnimationType::ANIMATION_ATTACK, {0, 10, 90}},
                {AnimationType::ANIMATION_ATTACK2, {10, 10, 90,}},
                {AnimationType::ANIMATION_ATTACK3, {20, 10, 90,}},
                {AnimationType::ANIMATION_BATTLE_IDLE, {62, 10, 100}},
                {AnimationType::ANIMATION_DAMAGED, {56, 9, 100, nullptr, [this](){ChangeAnimation(AnimationType::ANIMATION_IDLE);},nullptr}},
                {AnimationType::ANIMATION_DEATH, {30, 10, 100, nullptr, [this](){ChangeAnimation(AnimationType::ANIMATION_DEAD);},nullptr}},
                {AnimationType::ANIMATION_DEAD, {39, 1, 1000}},
                {AnimationType::ANIMATION_DIZZY, {40, 16, 100}},
                {AnimationType::ANIMATION_IDLE, {72, 16, 50}},
                {AnimationType::ANIMATION_IDLE2, {88, 8, 50}},
                {AnimationType::ANIMATION_BATTLE_IDLE, {65, 7, 50}},
                {AnimationType::ANIMATION_STATIC, {79, 1, 100}},
                {AnimationType::ANIMATION_RUN, {96, 6, 80}},
                {AnimationType::ANIMATION_SENSE_SOMETHING, {102, 48, 40}},
                {AnimationType::ANIMATION_SENSE_SOMETHING_START, {150, 10, 40}},
                {AnimationType::ANIMATION_TAUNT, {160, 16, 100}},
                {AnimationType::ANIMATION_VICTORY, {176, 16, 100}},
                {AnimationType::ANIMATION_WALK, {96, 6, 80}}
        });
...
void AngryMushroom::Update(GameTime gameTime) {
        ...
        faceDirection = (player.position.x < position.x) ? FaceDirection::FACE_DIRECTION_LEFT : FaceDirection::FACE_DIRECTION_RIGHT_ANGRYMUSHROOM;
        ...
        if (GetCurrentAnimation() == AnimationType::ANIMATION_RUN && gameTime.TimeElapsed(nextMoveTime)) {
                ...
                } else if (faceDirection == FaceDirection::FACE_DIRECTION_RIGHT_ANGRYMUSHROOM && canMoveRight) {
                        // Move Right
                        position.x += moveDistance *4;
                        nextMoveTime = gameTime.NowAddMilliseconds(200);
                } else {
...
void AngryMushroom::LateUpdate(GameTime gameTime) { TickAnimation(gameTime);}
void AngryMushroom::Draw(sf::RenderWindow &window, GameTime gameTime) {
        // Draw Shadow
        float shadowX = collisionBox.left + (collisionBox.width/2) - assetManager.TextureLibraries["PrgUse"]->entries[9].texture.getSize().x / 2;
        float shadowY = position.y + 270; // +310 for better alignment
        IDraw::Draw(window, "PrgUse", 9, sf::Vector2f(shadowX, shadowY));

        // Draw Monster
        window.draw(texQuads, &assetManager.TextureLibraries["AngryMushroom"]->entries[GetTextureDrawIndex() + faceDirection].texture);
...
void AngryMushroom::CalculatePhysicsState(std::vector<Boundary> boundaries, GameTime gameTime) {
...
void AngryMushroom::TickAnimation(GameTime gameTime) { Mob::TickAnimation(gameTime); }
void AngryMushroom::Damaged(int amount, GameTime gameTime) {
        if (!IsDead())
                ChangeAnimation(AnimationType::ANIMATION_DAMAGED,true);

        Mob::Damaged(amount, gameTime);
}
void AngryMushroom::DamagePlayer(int amount) {
        // Apply damage to the player
        player->health.Damage(amount);
}
sf::Vector2f AngryMushroom::GetLeftDropDetectorPosition() {
...
sf::Vector2f AngryMushroom::GetRightDropDetectorPosition() {
...
void AngryMushroom::UpdateQuads() {
        // Get the current animation frame
        TextureEntry* entry = &assetManager.TextureLibraries["AngryMushroom"]->entries[GetTextureDrawIndex() + faceDirection];
